	...
	//add-manufacture
	  ...
	  public static void insertProduct(EntityProduct product ) {
	       ...
	       try {
	    		Connection con= database.getConnection();
	    		Statement st=con.createStatement();    	            
	            st.execute("INSERT INTO `product`(SKU,name,`type`,area,description,`cost`,manufacture,inventory,image,keyword) VALUES ('"+product.getSKU()+"','"+product.getName()+"','"+product.getType()+"','"+product.getArea()+"','"+product.getDescription()+"','"+product.getCost()+"','"+product.getManufacture()+"','"+product.getInventory()+"','"+product.getImage()+"','"+product.getKeyword()+"')");
	            System.out.println("Inserted successfully!");	 
